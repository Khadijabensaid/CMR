  ...
  <div class="dropdown-content">
    ...
    <div class="w3-col m7">
      ...
      <div class="w3-container w3-card w3-white w3-round  " style=" min-height: 250px;"><br>
        <h2 class="w3-center ">Tableau de Bord</h2><br>
        <hr class="w3-clear">
        ...
        <%
			...
			Mesdemandes = DataBaseMethodes.DemandesApprouvees();
        	if(Mesdemandes.size() > 0){  %>
		<input type="hidden" name="action" value="consult" />	
		

        <div class="p-3 mb-2 bg-info text-white text-center">Mes Demandes Avec Matrice Non Remplie : </div>
        <br> 	
				...
				<thead>
				 <tr role="row">
			       ...
			       <th role="columnheader">Id</th>
			        <th role="columnheader">Date de Validation</th>
				...
				<tbody>				      
					...
					<input type="hidden" name="id-demande" value="<%=Mesdemandes.get(i).getId() %>" />	
				        ...
				        <td role="cell"><%=Mesdemandes.get(i).getDate_validation() %></td> 
		...
		<%
					...
					%>
					<br><br><h5 style="color:red;" class="w3-center">Vous n'avez pas de demandes a afficher</h5><br><br><br><br><br><br>
        ...
        <%
			...
			List<Demande> DemandesAvecMatriceRmplie = new ArrayList<Demande>();
			DemandesAvecMatriceRmplie = DataBaseMethodes.DemandesAvecMatriceRemplie();
        	if(DemandesAvecMatriceRmplie.size() > 0){  %>
		<input type="hidden" name="action" value="showmatrice" />	
    
        <div class="p-3 mb-2 bg-info text-white text-center">Mes Demandes Avec Matrice Rmplie : </div>
        <br> 	
				...
				<tbody>
					...
					for (int i = 0; i < DemandesAvecMatriceRmplie.size(); i++) {
					%>
					<form action="ServiceInfoServlet" method="post">	
      				<input type="hidden" name="action" value="showmatrice" />	
					<input type="hidden" name="id-demande" value="<%=DemandesAvecMatriceRmplie.get(i).getId() %>" />	

					<tr role="row">
				        <td role="cell"><%=DemandesAvecMatriceRmplie.get(i).getId() %></td> 
				        <td role="cell"><%=DemandesAvecMatriceRmplie.get(i).getDate_validation() %></td> 
				        <td role="cell"><%=DemandesAvecMatriceRmplie.get(i).getType() %></td>
				        <td role="cell"><%=DemandesAvecMatriceRmplie.get(i).getEtat() %></td>
				        <td role="cell"><button type="submit" class="w3-button  w3-blue ">Consulter Matrice</button> </td>
				     </tr>
					</form>	
        ...
        <%
			...
			List<Demande> DemandesProchainCab = new ArrayList<Demande>();
        DemandesProchainCab = DataBaseMethodes.DemandesProgrammesDansProchainCAB();
        	if(DemandesProchainCab.size() > 0){  %>
		<input type="hidden" name="action" value="showmatrice" />	
    
        <div class="p-3 mb-2 bg-info text-white text-center">Demandes a descuter Prochain Cab : </div>
        <br> 	
				...
				<thead>
			        ...
			        <th role="columnheader">Date de Validation</th>
			        <th role="columnheader">Type de Demande</th>
			        <th role="columnheader">Date de CAB</th>
			        <th role="columnheader">Action</th>
				...
				<tbody>
					...
					for (int i = 0; i < DemandesProchainCab.size(); i++) {
					%>
					<form action="ServiceInfoServlet" method="post">	
      				<input type="hidden" name="action" value="showmatrice" />	
					<input type="hidden" name="id-demande" value="<%=DemandesProchainCab.get(i).getId() %>" />	

					<tr role="row">
				        <td role="cell"><%=DemandesProchainCab.get(i).getId() %></td> 
				        <td role="cell"><%=DemandesProchainCab.get(i).getDate_validation() %></td> 
				        <td role="cell"><%=DemandesProchainCab.get(i).getType() %></td>
				        <td role="cell">date de cab</td>
				        <td role="cell"><button type="submit" class="w3-button  w3-blue ">Consulter Matrice</button> </td>
				     </tr>
					</form>	
        ...
        <%
			...
			List<Demande> DemandesAffectees = new ArrayList<Demande>();
        DemandesAffectees = DataBaseMethodes.DemandesAffectees();
        	if(DemandesAffectees.size() > 0){  %>
		<input type="hidden" name="action" value="showmatrice" />	
    
        <div class="p-3 mb-2 bg-info text-white text-center">Demandes Affectees : </div>
        <br> 	
				...
				<thead>
			        ...
			        <th role="columnheader">Date de Validation</th>
			        <th role="columnheader">Type de Demande</th>
			        <th role="columnheader">Date d'Affectation</th>
				...
				<tbody>
					...
					for (int i = 0; i < DemandesAffectees.size(); i++) {
					%>
					<form action="ServiceInfoServlet" method="post">	
      				<input type="hidden" name="action" value="showmatrice" />		

					<tr role="row">
				        <td role="cell"><%=DemandesAffectees.get(i).getId() %></td> 
				        <td role="cell"><%=DemandesAffectees.get(i).getDate_validation() %></td> 
				        <td role="cell"><%=DemandesAffectees.get(i).getType() %></td>
				        <td role="cell">date de cab</td>
				        <td role="cell"><button type="submit" class="w3-button  w3-blue ">Consulter Matrice</button> </td>
				     </tr>
					</form>	
