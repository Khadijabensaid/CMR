...
<!DOCTYPE html>
<html>
<title>Compte Rendu du CAB</title>
<meta charset="UTF-8">
        ...
        <br> 	
         ...
         <%
			...
			<form action="ServiceInfoCAB" method="post">
				...
				<input type="hidden" name="idCab" value="<%=cab.getId() %>" />
				<input type="hidden" name="listSize" value="<%=nv_demandes.size() %>" />	
				<table class="table"  >
				<thead>
				 <tr>
				...
				<tbody>
					...
					<tr role="row">	
					<% String decisionName = "decision"+ (i+1);
					   String respoName ="respo" +(i+1);
					   String idName ="id" +(i+1);
					
					%>		
						<input type="hidden" name=<%=idName %> value="<%=nv_demandes.get(i).getId() %>" />		
				        <td role="cell">Demande d 'Id :<%=nv_demandes.get(i).getId() %> </td> 
				         <td role="cell">
				         	<select id="type-compte" name="<%=decisionName %>" classe="w3-center">
				         ...
				         <td role="cell"> <textarea name="<%=respoName %>" rows="1" cols="25" style="resize: vertical;" required></textarea></td>
